...
#   The acquired data are sinkd/logged in the specified sink(s).
#
...
my $self = {};
    ...
    $self->{ CALLBACKMQ } = $self->curry( 'handler' );
    $self->{ CALLBACKCA } = $self->curry( 'caCallbcak' );
    return $self;
}
...
my $self = shift;
    ...
    if( @_ ){
        ...
        if( 2 == scalar( @_ ) ){
            my( $sinkType, $sinkId ) = @_;
            print "Adding sink: $sinkType => $sinkId \n";
            my %sink = ( $sinkType => $sinkId );
...
my $broker = shift;
    ...
    $mqttClient->subscribe( $self->{ 'topic' }, $self->{ 'CALLBACKMQ' } );
...
my $message = shift;
    ...
    #   We need to expand on this to allow multiple *sinks*.
    #
    if( $sinkType eq 'file' ){
